  ...
  function sortTable(table, col, reverse) {
    ...
    tRows = tRows.sort(function(a, b) {
      ...
      if (tempA.slice(-1) === "%" || tempB.slice(-1) === "%") {
        (tempA.slice(-1) === "%") && (tempA = tempA.slice(0, tempA.length - 1));
        (tempB.slice(-1) === "%") && (tempB = tempB.slice(0, tempB.length - 1));
        [tempA, tempB] = fixComparison(tempA, tempB);
      } else if (tempA.substr(0, 1) === "$" || tempB.substr(0, 1) === "$") {
        (tempA.substr(0, 1) === "$") && (tempA = tempA.slice(1, tempA.length));
        (tempB.substr(0, 1) === "$") && (tempB = tempB.slice(1, tempB.length));
        [tempA, tempB] = fixComparison(tempA, tempB);
      } else if (
        IS_NUM.test(stripCommas(tempA)) || IS_NUM.test(stripCommas(tempB))
      ) {
        (IS_NUM.test(stripCommas(tempA))) && (tempA = stripCommas(tempA) + "");
        (IS_NUM.test(stripCommas(tempB))) && (tempB = stripCommas(tempB) + "");
        [tempA, tempB] = fixComparison(tempA, tempB);
      }
